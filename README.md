...
Your 3DS must already have homebrew access. This ZIP does not modify the console.
Turn off the 3DS. Copy the 3ds folder from this ZIP to the root of its SD card.
Merge it with the existing folder if asked. Do not delete your existing apps.
Put the SD card back, turn on the 3DS, and open Homebrew Launcher.
Open Roku Remote 3DS.
...
Connect your Roku
Turn on your Roku. Connect it and your 3DS to the same home network.
They can use different Wi-Fi bands if the router allows them to communicate.
Avoid isolated guest networks.
On the Roku, find Settings > System > Advanced system settings >
...
Find the Roku's IP under Settings > Network > About.
In the 3DS app, choose Enter Roku IP and enter that address.
Choose Test connection, then Back to remote. Try Home.
The example IP in the screen preview is fictional; enter your own Roku's IP.
The app saves only that IP at `sdmc:/3ds/RokuRemote3DS/roku\_ip.txt`.
...
Linux host tests require GCC and Python 3: `python3 tests/test\_ecp.py`.
Validation
